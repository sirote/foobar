...
======================
You need to free the bunny workers before Commander Lambda's space station
explodes! Unfortunately, the Commander was very careful with the highest-value
workers -- they all work in separate, maximum-security work rooms. The rooms
are opened by putting keys into each console, then pressing the open button on
each console simultaneously. When the open button is pressed, each key opens
its corresponding lock on the work room. So, the union of the keys in all of
the consoles must be all of the keys. The scheme may require multiple copies of
one key given to different minions.

The consoles are far enough apart that a separate minion is needed for each
one. Fortunately, you have already relieved some bunnies to aid you - and even
...
work room. You know that Command Lambda wouldn't issue more keys than necessary
(beyond what the key distribution scheme requires), and that you need as many
bunnies with keys as there are consoles to open the work room.

Given the number of bunnies available and the number of locks required to open
a work room, write a function solution(num_buns, num_required) which returns a
...
list of the numbers for the keys. To cover all of the bunnies, the final
solution is represented by a sorted list of each individual bunny's list of
keys. Find the lexicographically least such key distribution - that is, the
...
no single bunny would be able to do it. Thus, the solution would be:
[
[0, 1],
[0, 2],
[1, 2],
...
def solution(num_buns, num_required):
    """Return a specification of how to distribute the keys."""
    spec = _get_intermediate_spec(num_buns, num_required)
    return _get_final_spec(spec)


def _get_intermediate_spec(num_buns, num_required):
    return [
        [
            None if bun in buns else key
            for bun in range(num_buns)
        ]
        for key, buns in enumerate(
            combinations(range(num_buns), num_required - 1)
        )
    ]


def _get_final_spec(spec):
    max_key = len(spec) - 1
    return [
        [
            max_key - key
