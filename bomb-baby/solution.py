...
def solution(mach_bombs, facula_bombs):
    """Find the fewest number of generations to create the given number
    of bombs.
    """
    bomb1, bomb2, cycle = int(mach_bombs), int(facula_bombs), 0
    while True:
        if bomb1 < 1 or bomb2 < 1:
            break

        if bomb1 == 1 or bomb2 == 1:
            return str(cycle + max(bomb1, bomb2) - 1)

        if bomb1 < bomb2:
            bomb1, bomb2 = bomb2, bomb1
...
if __name__ == '__main__':
    ...
    assert solution(str(10**50), '1') == str(10**50 - 1)
